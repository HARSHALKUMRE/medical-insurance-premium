...
def start_batch_prediction(input_file_path):
    try:
        ...
        """
        We need to create label encoder object for each categorical variable.
        We will check later.
        """
        ...
        prediction_file_name = os.path.basename(input_file_path).replace(".csv", f"{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv")
        prediction_file_path = os.path.join(PREDICTION_DIR, prediction_file_name)
        df.to_csv(prediction_file_path, index=False, header=True)
        logging.info(f"Prediction saved to: {prediction_file_path}")
