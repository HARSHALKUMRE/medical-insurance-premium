...
# 80 accuracy running
# 65, 68, 70 new models created
# above pr equal to 70 then we can accept else we can reject (threshold)
...
class ModelTrainer:
    ...
    def initiate_model_trainer(self,) -> artifact_entity.ModelTrainerArtifact:
        try:
            ...
            if r2_test_score<self.model_trainer_config.excepted_score:
                raise Exception(f"Model is not good as it is not able to give \
                expected accuracy: {self.model_trainer_config.excepted_score}: model actual score: {r2_test_score}")
